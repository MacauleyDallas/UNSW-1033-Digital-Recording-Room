...
Version     Date        Notes
-------     ----        -----
1.0.0       2/1/2023    Initial release.
"""
...
class WatchVariable(_ManualEventBase):
    ...
    with the ``extronlib.event`` and :py:attr:`eventEx` decorators.
    ...
    Use this class to signal, in a way compatible with the extronlib @event decorator, other parts
    of your program that the state of a system variable has been changed.

    Examples
    --------
    In the variable definition portion of the ControlScript program:
    ::

        CallStatus = 'On Hook'
        CallStatusWatch = WatchVariable(callstatus, 'Video call status')

    In the system definition portion of the ControlScript program:
    ::

        from variables import CallStatus, CallStatusWatch

        @event(CallStatusWatch, 'Changed')
    ...
    def Changed(self):
        """
        ``Event`` - Triggers when this instance's :py:attr:`Change` is called.
    ...
    def Change(self, *args, **kwargs):
        ...
        It accepts a variable number of positional and keyword arguments which, along with
        this instances, are passed to the Changed handler.
